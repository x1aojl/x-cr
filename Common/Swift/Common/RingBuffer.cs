...
namespace Swift
{
    ...
    /// </summary>
    public class RingBuffer : IReadableBuffer, IWriteableBuffer, ICloneable
    {
        public RingBuffer()
            : this(false, false)
        {
        ...
        public bool PeekBool(ref bool v)
        {
            lock (this)
            {
                int cnt = sizeof(bool);
                ...
                {
                    v = Peek<bool>(BitConverter.ToBoolean, cnt);
                    return true;
                }
            }
        ...
        public bool PeekInt(ref int v) { return PeekInt(0, ref v); }
        public bool PeekInt(int offset, ref int v)
        {
            lock (this)
            {
                int cnt = sizeof(int);
                if (available < cnt + offset)
                    return false;
                else
                {
                    v = ReadFromNetOrder
                         ? IPAddress.NetworkToHostOrder(Peek<int>(BitConverter.ToInt32, offset, cnt))
						 : Peek<int>(BitConverter.ToInt32, offset, cnt);
                    return true;
                }
            }
        }
        ...
        public byte[] PeekBytes(int cnt)
        {
            ...
            if (rPos + cnt > data.Length)
            {
                ...
                Array.Copy(data, rPos, tmp, 0, cnt);

            return tmp;
        }

        // 从当前读位置拾取一个数据，但不改变缓冲区状态
        T Peek<T>(Func<byte[], int, T> converter, int typeSize) { return Peek(converter, 0, typeSize); }
        T Peek<T>(Func<byte[], int, T> converter, int offset, int typeSize)
        {
            int cnt = typeSize;
            if (cnt + offset > available)
                throw new Exception("RingBuffer peeking overflow");

            if (rPos + offset + cnt > data.Length)
            {
                byte[] tmp = new byte[cnt];
                int len1 = data.Length - rPos - offset;
                int len2 = cnt - len1;
                Array.Copy(data, rPos + offset, tmp, 0, len1);
                Array.Copy(data, 0, tmp, len1, len2);
                return converter(tmp, 0);
            }
            else
                return converter(data, rPos + offset);
        }
        ...
        public Object Clone()
        {
            var buff = new RingBuffer(ReadFromNetOrder, WriteToNetOrder, data);
            buff.rPos = rPos;
