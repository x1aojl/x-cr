...
namespace Swift
{
    ...
    public static class FC
    {
        ...
        {
            if (start <= end)
                For(start, end + 1, f, continueCondition);
            else
                For(end, start + 1, f, continueCondition);
        }
        ...
        public static void ObliqueSquareFor(int cx, int cy, int r, Action<int, int> f)
        {
            if (r < 0)
                return;
        ...
        public static void ObliqueSquareFor2(int cx, int cy, int start_r, int end_r, Action<int, int> f)
        {
            for (int r = start_r; r < end_r; r++)
                ObliqueSquareFor(cx, cy, r, f);
        }

        public static bool Contains<T>(IEnumerable<T> itor, Func<T, bool> f)
        {
        ...
        // 满足条件的对象映射为新列表
        public static List<T> Select<T>(IEnumerable<T> origin, Func<T, bool> filter)
        {
            var lst = new List<T>();
