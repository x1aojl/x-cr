...
namespace Server
{
    ...
    public class MySqlDbPersistence<T, IDType> : IAsyncPersistence<T, IDType> where T : DataItem<IDType>
    {
            ...
            Func<T, byte[]> data2BuffHandler, Func<byte[], T> buff2DataHandler, Func<T, string, object> colValueMap)
        {
            ...
            MakeSureTableExists(tbName, createTableCommand);
        }

        // 检查指定表是否存在
        void MakeSureTableExists(string tableName, string createTableCommand)
        {
        ...
        // 载入指定条件的数据项
        public void LoadAll(Action<T[]> cb, string whereClause)
        {
            MySqlConnection conn = null;
            MySqlDataReader r = null;
            MySqlCommand cmd = null;
